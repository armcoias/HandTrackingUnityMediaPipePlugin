...
{   
    public class HandTracking : MonoBehaviour
    {
        ...
        private IEnumerator Start()
        {
            ...
            var handLandmarksSream = new OutputStream<NormalizedLandmarkListVectorPacket, List<NormalizedLandmarkList>>(_graph, "hand_landmarks");
            outputVideoStream.StartPolling().AssertOk();
            handLandmarksSream.StartPolling().AssertOk();
            ...
            while (true)
            {
                ...
                if (handLandmarksSream.TryGetNext(out var handLandmarks))
                {
                    if (handLandmarks != null && handLandmarks.Count > 0)
                    {
                        foreach (var landmarks in handLandmarks)
                        {
                            ...
                            Debug.Log($"Unity Local Coordinates: {screenRect.GetPoint(wrist)}, Image Coordinates: {wrist}");
                        }
                    }
                }
            }
